...
if __name__ == "__main__":
    ...
    for stage in range(opt.num_nets):
        ...
        # Train
        tr_rmse, _ = root_mse(net_ensemble, train_loader)
        if opt.cv:
            val_rmse, _ = root_mse(net_ensemble, val_loader) 
            if val_rmse < best_rmse:
                best_rmse = val_rmse
                best_stage = stage

        te_rmse, out = root_mse(net_ensemble, test_loader)
    ...
    out_csv.to_csv(opt.filename, index=False)
